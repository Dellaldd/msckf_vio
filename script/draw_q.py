...
def main():
    ...
    file_name = "record_3.txt"
    save_name = "record_3.png"
    ...
    T_imu0_gt0[1,1] = -1
    T_imu0_gt0[0,0] = -1
    print(np.linalg.inv(T_imu0_gt0))
    ...
    for i in range(first_index, data.shape[0]):
        ...
        T_gt_gt0 = np.dot(np.linalg.inv(T_gt0_w), T_gt_w)
        # T_gt_gt0 = np.dot(np.dot(T_imu0_gt0, T_gt_gt0), T_imu0_gt0)
        ...
        T_imu_gt0 = np.dot(np.dot(np.dot(T_imu0_gt0, np.linalg.inv(T_imu0_w)),T_imu_w ),np.linalg.inv(T_imu0_gt0))
        # T_imu_gt0 = np.dot(np.dot(T_imu0_gt0 ,T_imu_w ),np.linalg.inv(T_imu0_gt0))
        # T_imu_gt0 = T_imu_imu0
    ...
    fig.legend()
    fig.tight_layout()
    save_path = "/home/ldd/msckf_real/src/msckf_vio/path/" + save_name
